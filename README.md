...
[딥씽크카 딥러닝 트레이닝](https://jd-edu.github.io/deepThinkCar_mini/step3)
...
압축을 위해 "jd_label_data_compress.py" 스크립트를 실행하면 "car_image_angle.zip"이라는 압축파일이 하나 생깁니다.    
이 파일을 주피터 노트북을 이용해서 PC로 옮겨야 합니다. 이를 위해 먼저 주피터 노트북을 다음과 같이 실행합니다. deepThinkCar에 주피터노트북셋업은 생략되어 있습니다. 
만약 주피터 노트북 설치가 어려우면 USB 메모리 스틱을 사용해도 됩니다. 
<pre><code>
...
<pre><code>
C:\Users\user\Downloads>git clone https://github.com/JD-edu/deepThinkCar_PC.git
Cloning into 'deepThinkCar_PC'...
...
deepThinkCar 아나코다 가상환경을 다운로드 했으면, YML 파일을 이용하여 가상환경을 설치합니다. 다운로드 받은 deepThinkCar용 아나콘다 가상화경에 보면 "jdlab.yml"이라는 파일이 있습니다. 이 YML 환경파일을 이용해서 deepThinkCar용 아나콘다 가상환경을 만들 수 있습니다.
...
<pre><code>
(base) PS C:\Users\user\Downloads> cd .\deepThinkCar_PC\
(base) PS C:\Users\user\Downloads\deepThinkCar_PC> ls


    디렉터리: C:\Users\user\Downloads\deepThinkCar_PC
...
(base) PS C:\Users\user\Downloads\deepThinkCar_PC>
</code></pre>

이 중에서 "jdlab.yml" 환경파일을 이용해서 deepThinkCar용 아나콘다 가상환경을 만들게 됩니다. YML 환경파일을 확인 했으면 다음과 같이 명령을 입력합니다. 
<pre><code>
...
(jdlab_v10) PS C:\Users\user\Downloads\deepThinkCar-tf-PC>
</code></pre>   
...
<pre><code>
(jdlab_v10) PS C:\Users\user\Downloads\deepThinkCar+PC\data> ls


    디렉터리: C:\Users\user\Downloads\deepThinkCar_PC\data
...
_________________________________________________________________
None
WARNING:tensorflow:From .\jd_deep_learning.py:198: Model.fit_generator (from tensorflow.python.keras.engine.training) is deprecated and will be removed in a future version.
